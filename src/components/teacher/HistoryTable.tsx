...
export default function HistoryTable({
  ...
  const filteredAttempts = safeAttempts.filter((a) => {
    const userId = (a.userId || '').toLowerCase();
    const examTitle = (a.examTitle || '').toLowerCase();
    const userName = (a.userName || '').toLowerCase();
    const q = (searchTerm || '').toLowerCase();
    const matchesSearch = userId.includes(q) || examTitle.includes(q) || userName.includes(q);
    const matchesClass =
      selectedClass === 'All' || (a.className || 'Chưa xác định') === selectedClass;
        ...
        <div className="p-24 text-center">
          ...
          <p className="text-slate-400 font-medium">
            {safeAttempts.length === 0
              ? 'Chưa có bài nộp nào. Học sinh cần hoàn thành bài thi trước.'
              ...
              {filteredAttempts.map((attempt) => (
                <tr key={attempt.id} className="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 font-bold overflow-hidden">
                        {(attempt.userName || attempt.userId || '?').charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <div className="font-bold text-slate-900">
                          {attempt.userName || attempt.userId || 'Ẩn danh'}
                        </div>
                        <div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">
                          Lớp: {attempt.className || 'Chưa xác định'}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-8 py-6">
                    ...
                    <div className="text-[10px] text-slate-400 mt-1">{formatDate(attempt.date)}</div>
                  </td>
